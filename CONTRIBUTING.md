...
Core source and colocated tests are grouped under `content/`, `drafts/`, `publishing/`,
...
before each UI command. This differs from `packages/core/src/identity/auth-schema.ts`, which is generated by
...
`./packages/core/src/identity/auth-schema.ts` as its explicit output. Commit the generated schema and check
...
test. Mock only at boundaries (network, clock, DB). The default suite runs locally, even when
`.env.test` exists. To run the live GitHub contract tests, copy `.env.test.example` to `.env.test`
at the repo root and point it at a private throwaway repo the App is installed on, then run:

```sh
pnpm --filter @handover/core test:integration
```

That command explicitly enables tests which commit to the configured repository and clean up
their fixture files afterward.
...
`FORMAT_VERSION` in `packages/core/src/content/entry-format.ts` — which in turn needs the `from: N` step
in `content/migrate.ts` that `migrate.test.ts` checks for. Adding a golden is free. The check is
