...
test('an entry nobody is editing is taken, and the expiry is one lifetime out', async () => {
  expect(await claimLock('default', db, 'listings/seaview-cottage', 'u1', NOW)).toBe(
    NOW + LOCK_TTL,
  );
...
test('a beat from the holder pushes their own lock further out', async () => {
  ...
  expect(await claimLock('default', db, 'listings/seaview-cottage', 'u1', NOW + 45_000)).toBe(
    NOW + 45_000 + LOCK_TTL,
  );
});
...
test('a second editor is refused, and reads who is editing it', async () => {
  await claimLock('default', db, 'listings/seaview-cottage', 'u1', NOW);
  expect(
    await claimLock('default', db, 'listings/seaview-cottage', 'u2', NOW + 1000),
  ).toBeUndefined();
...
test('a lock is held up to its expiry and taken from the moment it passes', async () => {
  ...
  expect(
    await claimLock('default', db, 'listings/seaview-cottage', 'u2', expiry - 1),
  ).toBeUndefined();
  expect(await lockHolder('default', db, 'listings/seaview-cottage', expiry)).toBeUndefined();
  expect(await claimLock('default', db, 'listings/seaview-cottage', 'u2', expiry)).toBe(
    expiry + LOCK_TTL,
  );
});
...
test('what somebody is editing leaves out the entries their beat has run out on', async () => {
  await claimLock('default', db, 'listings/seaview-cottage', 'u1', NOW);
  await claimLock('default', db, 'pages/home', 'u1', NOW);
  await claimLock('default', db, 'pages/about', 'u2', NOW - LOCK_TTL);
...
test('removing a member lets go of their entries and leaves everyone else holding theirs', async () => {
  await claimLock('default', db, 'listings/seaview-cottage', 'u1', NOW);
  await claimLock('default', db, 'pages/home', 'u2', NOW);
