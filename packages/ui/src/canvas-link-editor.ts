...
export function createCanvasLinkEditor(options: CanvasLinkEditorOptions = {}) {
  ...
  let directory: LinkEntry[] | undefined;
  let directoryRequest: Promise<LinkEntry[]> | undefined;
  ...
  const loadDirectory = () => {
    directoryRequest ??= owner
      .fetch('/admin/api/entries')
      .then(async (response) => {
        if (!response.ok) return [];
        const value = (await response.json()) as LinkDirectory;
        return [...(value.indexes ?? []), ...(value.entries ?? [])];
      })
      .catch(() => []);
