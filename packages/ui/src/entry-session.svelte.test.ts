...
test('a nested list resolves through stable parents and rejects referenced descendants', () => {
  ...
  const replacement = JSON.parse(JSON.stringify(session.snapshot('en'))) as typeof nestedData;
  const card = (replacement.blocks as typeof nestedData.blocks)[0];
  if (!card) throw new Error('card block missing');
  card._ref = 'globals/card';
  session.replaceSnapshot('en', replacement);
  expect(
    session.listCommand('en', {
      address: 'blocks[_id=card].items',
      contentVersion: 2,
...
test('contiguous typing is one reversible transaction with its logical selection', () => {
  vi.useFakeTimers();
...
test('a new change clears redo and an external locale replacement drops obsolete history', () => {
  const session = createEntrySession({
    sourceLocale: 'en',
    data: widgetData(),
    translations: { de: widgetData() },
  ...
  expect(session.canRedo()).toBe(false);
  session.replaceSnapshot('en', { ...widgetData(), title: 'External source' });
  expect(session.canUndo()).toBe(false);
});
