...
import type { CanvasInteractionMode, CanvasNavigationRequest } from './canvas-navigation';
import type { FieldCommandFailure } from './entry-session.svelte';
...
const sameLocation = (a?: CanvasLocation, b?: CanvasLocation) =>
  a === b ||
  (!!a &&
    !!b &&
...
const stale = (
  ...
  if (!sameDocument(message.entry, manifest.entry)) return 'stale-document';
...
export function createCanvasParentBridge(options: CanvasParentBridgeOptions) {
  ...
  const receive = (event: MessageEvent) => {
    ...
    if (requestedAction) {
      ...
      if (!current || !sameTarget(requestedAction.selection.target, current))
    ...
    if (editing) {
      ...
      const current = options.currentTarget();
      if (!current || !sameTarget(editing.target, current)) return reject('stale-target', editing);
    ...
    const current = options.currentTarget();
    if (!current || !sameTarget(message.target, current)) return refusal(message, 'stale-target');
...
export function createCanvasChildBridge(options: CanvasChildBridgeOptions) {
  ...
  const acceptReply = (value: unknown) => {
    ...
    if (
      ...
      !sameTarget(reply.target, held.target)
    )
      return;
...
export function readCanvasResultManifest(
  ...
  try {
    ...
    if (value.status === 'success' && keys(value, ['mode', 'status', ...BASE]) && identity(value))
