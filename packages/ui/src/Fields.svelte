...
let {
  ...
  root = $bindable(),
  path = [],
}: { fields: readonly Field[]; root: Data; path?: readonly string[] } = $props();
...
{#each fields as field (field.path.join('.'))}
  ...
  {@const id = `f-${at.join('.')}`}
  {@const text = capitalise(field.path[0] ?? '')}
  <div class="field">
    {#if field.type === 'text'}
    ...
    {:else if field.type === 'link'}
      <div class="label-row"><span id="{id}-l">{text}{#if field.required}<span class="req" aria-hidden="true">*</span>{/if}</span></div>
    ...
    {:else if field.type === 'group'}
      <details class="group" open>
        ...
        <div class="form"><Fields fields={field.fields} bind:root path={at} /></div>
      </details>
    {:else}
