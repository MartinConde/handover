...
const sameTarget = (a: CanvasTarget, b: CanvasTarget) =>
  ...
  ((!a.occurrence && !b.occurrence) ||
    (!!a.occurrence &&
      !!b.occurrence &&
      sameDocument(a.occurrence.document, b.occurrence.document) &&
      a.occurrence.locale === b.occurrence.locale &&
      a.occurrence.address === b.occurrence.address));

const sameSelection = (a: CanvasSelection | undefined, b: CanvasSelection | undefined) =>
  a === b || (!!a && !!b && a.kind === b.kind && sameTarget(a.target, b.target));
...
const isAncestor = (parent: StructuralLocation, child: StructuralLocation) => {
  if (
    !sameDocument(parent.document, child.document) ||
...
const selectionKey = (kind: CanvasAnnotationKind, target: CanvasTarget) =>
...
function readStructure(root: Document): InternalNode[] {
  ...
  for (const element of Array.from(root.querySelectorAll(SELECTOR))) {
    for (const [attribute, kind] of MARKERS) {
      ...
      if (!target) continue;
      const key = selectionKey(kind, target);
...
export function createCanvasSelectionRuntime(options: CanvasSelectionRuntimeOptions = {}) {
  ...
  const nodeForSelection = (value: CanvasSelection | undefined) =>
    value && nodeByKey.get(selectionKey(value.kind, value.target));
  ...
  const draw = () => {
    ...
    if (
      !editing &&
      hoveredNode &&
      (!selectedNode || !sameSelection(hoveredNode, selectedNode)) &&
  ...
  const indexNodes = () => {
    nodeByKey = new Map(nodes.map((node) => [selectionKey(node.kind, node.target), node]));
  ...
  const choose = (
    ...
    if (!sameSelection(selected, value)) {
      allowedActions = [];
      actionsOpen = false;
    }
  ...
  const onDoubleClick = (event: MouseEvent) => {
    ...
    if (!sameSelection(node, selected)) choose(node, element);
  ...
  return {
    ...
    actions(value: CanvasSelection, values: CanvasBlockAction[]) {
      if (!sameSelection(selected, value)) return false;
