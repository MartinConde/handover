...
/** Whether this source change can alter the locale-owned value of this one field. */
const localeFieldProjectionNeeded = (
  context: FieldContext,
  changes: readonly { path: readonly string[] }[],
) => {
  if (context.mode === 'duplicate') return true;
  if (context.mode !== true) return false;
  const translated = TRANSLATED_PROPS[context.field.type];
  return (
    translated !== undefined &&
    changes.some((change) => !change.path.length || !translated.includes(change.path.join('.')))
  );
};
...
export function createEntrySession({
  ...
  const recordHistory = (
    ...
    if (current?.type === 'field' && (groupedTyping || groupedComposition)) {
      const compactTyping =
        groupedTyping &&
        current.forward.length === 1 &&
        current.inverse.length === 1 &&
        forward.length === 1 &&
        inverse.length === 1 &&
      ...
      if (compactTyping) {
        ...
        current.forward[0] = forward[0] as HistoryChange;
      } else {
  ...
  /** Field edits cannot change row presence, so resolve and project only their stable owner. */
  const projectFieldLocales = (address: string, context: FieldContext, source: EntryData) => {
    if (!form) return;
    const sourceValue = read(source, context.path);
    for (const locale of locales()) {
      if (locale === sourceLocale) continue;
      const target = snapshots[locale];
      if (!target) continue;
      const resolved = resolveFieldTarget('default', form, address, target);
      if (!resolved.ok) continue;
      const targetContext = fieldContext(form, resolved.target.path, target);
      if (!targetContext) continue;
      const before = read(target, targetContext.path);
      const projected = syncLocaleField(context.field, context.mode, sourceValue, before);
      if (equal(before, projected)) continue;
      write(target, targetContext.path, clone(projected));
      mutated(locale);
    }
  };
  ...
  const runFieldCommand = (locale: string, command: FieldCommand): FieldCommandResult => {
    ...
    const changed = changes.some(
      (change) => !equal(change.value, read(current, [...context.path, ...change.path])),
    );
    if (!changed) {
      if (command.history?.after) setHistorySelection(command.history.after);
      return { ok: true, contentVersion: beforeVersion };
    }
    ...
    if (locale === sourceLocale && localeFieldProjectionNeeded(context, changes))
      projectFieldLocales(ownerAddress, context, current);
  ...
  return {
    ...
    /** Shallow history counts used to verify and profile value retention. */
    historyStats(): HistoryStats {
      const retainedFieldValues = [...undoStack, ...redoStack].reduce(
        (total, transaction) =>
          transaction.type === 'field'
            ? total + transaction.forward.length + transaction.inverse.length
            : total,
        0,
      );
      return {
    ...
    /** Reserve the entry, drain every locale, then own checks and the final commit on the lane. */
    async finalPublish(
      request: () => Promise<boolean>,
      reload: (outcome: 'published' | 'uncertain') => void | Promise<void>,
