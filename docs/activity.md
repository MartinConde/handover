...
| Entries | `entry-rename` and `entry-duplicate`, each with the name it had before or was copied from · `entry-delete` and `locale-off`, each with the commit that took the files away and the languages that went — which is what a **Restore** on the row puts back ([Entry lifecycle](entry-lifecycle.md#putting-a-deleted-entry-back)) · `sources-recorded`, the one commit that recorded the source language of older entries, with how many · `entry-source`, the language an entry is written in changed, `{ from, to }`; nothing is committed until it publishes |
