...
**Status** filter shows the hidden rows on their own, or only the live ones; the heading
then counts what is shown, of the total (*2 of 14*).
...
same commit as `_status` ([Redirects](redirects.md)). Showing the entry
...
[Redirects](redirects.md), together with globals, navigation and templates.
