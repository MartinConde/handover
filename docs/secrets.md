...
| `DEEPL_API_KEY` | [machine translation](machine-translation.md#a-machines-first-draft) from the admin — a free key ends in `:fx` and is recognised as one. Without it the translate buttons are not drawn. The owner can paste a key of their own in **Settings**, which overrides this one ([Translation and AI](diagnostics.md#translation-and-ai)) |
