...
where the last build found the translation behind its source, half filled where the file answers
only part of the source's text ([How much is written](translating.md#how-much-is-written)), and
not drawn at all on a one-language site. A file both behind and partly written is tinted. An entry written in one language only is listed by the words it has, whichever
...
to three chips for the languages still owed (missing, partly written or stale) in the site's order, and `+2` when
...
and the filter beside it says which work: **Missing, partly written or stale** (the default),
**Missing** (no file yet), **Partly written** (a file that answers only part of the source),
**Stale** (a translation the last build found behind its source) or **Machine translated** (a
file that still holds text a machine wrote). A file both stale and partly written is in both
lists. Machine translated is not owed by itself, so the default leaves it out. A language turned
off for an entry is never owed in it. Partly written and machine translated count unpublished
changes; stale is the last build's. The second filter is off until a language is chosen. The
...
translations. `owed` takes `missing`, `unfinished`, `stale` or `machine`; any other value means
everything owed, and a language the site does not declare filters nothing.
